...
async function generateEventContractPDF(form, signature, signedDate) {
  ...
  (form.additionalCosts || []).forEach((c) => {
    ...
    if (c.description || amt > 0) {
      svcItems.push({ name: c.description || "Additional Item", rate: "—", qty: "—", amt });
    }
  });
  ...
  body(
    discD > 0
      ...
      : "Deposit of 50% is due upon signing to reserve the date. Balance due no later than 14 days before the event. Payment accepted via Zelle, check, Venmo, card, or cash. Overtime beyond contracted hours: $150/hr.",
    0,
    [75, 85, 99]
  );
  if (discP > 0) {
